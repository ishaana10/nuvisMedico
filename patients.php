            ...
            <tbody class="divide-y divide-outline-variant/20">
                ...
                <?php foreach ($patients as $p): ?>
                    <tr class="hover:bg-surface-container-low/50 transition">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <?php else: ?>
                                    <div class="w-9 h-9 rounded-full bg-primary-fixed text-primary font-bold text-xs flex items-center justify-center">
                                        <?= htmlspecialchars($p['initials'] ?: substr($p['first_name'],0,1).substr($p['last_name'],0,1)) ?>
                                    </div>
                                <?php endif; ?>
                                <div>
                                    ...
                                    <p class="text-[11px] font-mono font-medium text-slate-500"><?= htmlspecialchars($p['mrn']) ?></p>
                                </div>
                            </div>
                        </td>
                        <td class="py-3.5 px-4 font-medium text-on-surface-variant">
                            <?= htmlspecialchars($p['age']) ?> yrs • <?= htmlspecialchars($p['gender']) ?>
                            <p class="text-[10px] text-outline">DOB: <?= htmlspecialchars($p['dob']) ?></p>
                        </td>
                        <td class="py-3.5 px-4 font-medium text-on-surface-variant">
                        ...
                        <td class="py-3.5 px-4">
                            <?php if (!empty($p['known_allergies']) && $p['known_allergies'] !== 'None' && $p['known_allergies'] !== 'None reported'): ?>
                                <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-amber-100 text-amber-800 text-[10px] font-bold">
                                    ...
                                    <?= htmlspecialchars($p['known_allergies']) ?>
                                </span>
                            <?php else: ?>
